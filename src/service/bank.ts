...
export const getBalances = async (address: string): Promise<Balance[]> => {
  try {
    const res = await axios.get(
      `${ENV.FIVENET_API}cosmos/bank/v1beta1/balances/${address}`
    );
...
export const getBalance = async (address: string): Promise<Balance | null> => {
  try {
    const res = await axios.get(
      `${ENV.FIVENET_API}cosmos/bank/v1beta1/balances/${address}/by_denom?denom=usix`
    );
...
export const getCoinMetadata = async (denom: string): Promise<any> => {
  try {
    const res = await axios.get(
      `${ENV.FIVENET_API}cosmos/bank/v1beta1/denoms_metadata/${denom}`
    );
